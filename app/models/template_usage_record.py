...
class TemplateUsageRecord(Base):
    ...
    reminder_id: Mapped[int | None] = mapped_column(ForeignKey("reminders.id"), nullable=True, comment="创建的提醒ID")
    used_at: Mapped[datetime] = mapped_column(server_default=func.now(), comment="使用时间")
    feedback_rating: Mapped[int | None] = mapped_column(nullable=True, comment="评分 1-5")
    feedback_comment: Mapped[str | None] = mapped_column(Text, nullable=True, comment="评价内容")
