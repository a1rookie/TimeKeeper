...
class Reminder(Base):
    ...
    family_group_id: Mapped[int | None] = mapped_column(ForeignKey("family_groups.id"), nullable=True, index=True, comment="家庭组ID")
    template_id: Mapped[int | None] = mapped_column(ForeignKey("reminder_templates.id"), nullable=True, comment="模板ID")
    ...
    last_remind_time: Mapped[datetime | None] = mapped_column(nullable=True, comment="上次提醒时间")
    ...
    family_group: Mapped["FamilyGroup"] = relationship(back_populates="reminders")
    template: Mapped["ReminderTemplate"] = relationship(back_populates="reminders")
