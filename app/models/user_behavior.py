...
class UserBehavior(Base):
    ...
    active_hours: Mapped[Optional[List[int]]] = mapped_column(type_=JSON, nullable=True, comment="活跃时段(JSON): [8, 9, 18, 19, 20]")
    confirm_avg_response_minutes: Mapped[Optional[int]] = mapped_column(nullable=True, comment="平均响应时间(分钟)")
    completion_rate: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), nullable=True, comment="完成率")
    most_used_categories: Mapped[Optional[List[str]]] = mapped_column(type_=JSON, nullable=True, comment="常用分类(JSON): ['rent', 'health']")
