...
class UserCustomTemplate(Base):
    ...
    recurrence_type: Mapped[str | None] = mapped_column(String(20), nullable=True, comment="周期类型")
    recurrence_config: Mapped[Dict[str, Any] | None] = mapped_column(type_=JSON, nullable=True, comment="周期配置")
    advance_days: Mapped[int | None] = mapped_column(nullable=True, comment="提前天数")
    category: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="分类")
    created_from_template_id: Mapped[int | None] = mapped_column(ForeignKey("reminder_templates.id"), nullable=True, comment="基于的系统模板ID")
    ...
    base_template: Mapped["ReminderTemplate"] = relationship(back_populates="custom_templates")
