...
class PushLog(Base):
    ...
    task_id: Mapped[int | None] = mapped_column(ForeignKey("push_tasks.id"), nullable=True, comment="推送任务ID")
    ...
    user_action: Mapped[str | None] = mapped_column(String(20), nullable=True, comment="用户操作: confirmed, ignored, delayed, dismissed")
    user_action_time: Mapped[datetime | None] = mapped_column(nullable=True, comment="用户操作时间")
    response_time_seconds: Mapped[int | None] = mapped_column(nullable=True, comment="响应时间(秒)")
    
    # Relationships
    task: Mapped["PushTask"] = relationship(back_populates="logs")
