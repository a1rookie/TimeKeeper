...
class SmsLog(Base):
    ...
    ip_address: Mapped[Optional[str]] = mapped_column(String(50), nullable=True, comment="请求IP地址")
    ...
    provider: Mapped[Optional[str]] = mapped_column(String(20), nullable=True, comment="短信提供商: aliyun, noop")
    error_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="错误信息")
    
    # 验证状态
    is_verified: Mapped[bool] = mapped_column(default=False, comment="是否已验证")
    verified_at: Mapped[Optional[datetime]] = mapped_column(nullable=True, comment="验证时间")
    ...
    sent_at: Mapped[Optional[datetime]] = mapped_column(nullable=True, comment="发送时间")
    expires_at: Mapped[Optional[datetime]] = mapped_column(nullable=True, index=True, comment="过期时间")
