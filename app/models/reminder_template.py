...
class ReminderTemplate(Base):
    ...
    default_recurrence_type = Column(String(20), comment="默认周期类型")
    default_recurrence_config = Column(JSON, comment="默认周期配置")
    default_advance_days = Column(Integer, default=3, comment="默认提前天数")
    suggested_amount = Column(Numeric(10, 2), comment="建议金额")
    suggested_attachments = Column(JSON, comment="建议附件")
    ...
    custom_templates = relationship("UserCustomTemplate", back_populates="base_template")
