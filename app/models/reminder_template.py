...
class ReminderTemplate(Base):
    ...
    default_recurrence_type: Mapped[Optional[str]] = mapped_column(String(20), nullable=True, comment="默认周期类型")
    default_recurrence_config: Mapped[Optional[Dict[str, Any]]] = mapped_column(type_=JSON, nullable=True, comment="默认周期配置")
    default_advance_days: Mapped[int] = mapped_column(default=3, comment="默认提前天数")
    suggested_amount: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 2), nullable=True, comment="建议金额")
    suggested_attachments: Mapped[Optional[List[Dict[str, Any]]]] = mapped_column(type_=JSON, nullable=True, comment="建议附件")
