...
class ReminderTemplateResponse(BaseModel):
    ...
    default_recurrence_config: dict | None
...
class UserCustomTemplateCreate(BaseModel):
    ...
    recurrence_type: str | None = Field(None, description="周期类型")
    recurrence_config: dict | None = Field(None, description="周期配置")
    remind_advance_days: int = Field(0, ge=0, description="提前提醒天数")
    created_from_template_id: int | None = Field(None, description="基于的系统模板ID")
...
class UserCustomTemplateUpdate(BaseModel):
    ...
    remind_advance_days: int | None = Field(None, ge=0)
...
class UserCustomTemplateResponse(BaseModel):
    ...
    created_from_template_id: int | None
    created_at: datetime
    updated_at: datetime
...
class TemplateShareCreate(BaseModel):
    ...
    share_description: str | None = Field(None, max_length=500, description="分享描述")
    family_group_id: int | None = Field(None, description="家庭组ID（仅家庭分享需要）")
...
class TemplateShareDetail(TemplateShareResponse):
    """模板分享详情（包含完整模板信息）"""
    template: UserCustomTemplateResponse | None = None
...
class TemplateUsageCreate(BaseModel):
    ...
    feedback_rating: int | None = Field(None, ge=1, le=5, description="评分 1-5")
    feedback_comment: str | None = Field(None, max_length=500, description="评价内容")
